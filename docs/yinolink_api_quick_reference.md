...
注意：YinoLink 在 `time_range` 跨多天且带 hourly breakdown 时，实测返回 24 个“小时-of-day”聚合桶，不是逐日逐小时明细。单日监控应按广告账户 `timezone_name` 算出明确账户日期，再使用 `time_range={"since":"YYYY-MM-DD","until":"YYYY-MM-DD"}`；`date_preset=today/yesterday` 只适合手工验证或显式覆盖。
...
2. 每 15 或 30 分钟拉一次重点广告指标：
   - `/meta_api/insights?id=<ad_id>&time_range={"since":"<account-day>","until":"<account-day>"}&breakdowns=hourly_stats_aggregated_by_advertiser_time_zone`
   - 用 SQLite upsert 覆盖同一 `ad_id + hour_start + hourly_range`。
   - 入库保留账户时区小时桶，并额外写 `hour_start_beijing` 给前端显示。
